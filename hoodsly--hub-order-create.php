...
final class HoodslyHub {
	...
	function test_order_data() {
		...
		foreach ( $order->get_items() as $item_key => $item_values ) {
			...
			$product_img_url = $product->get_image();
			//preg_match_all('~src=["\']+(.*?)["\']+~', $product_img_url, $image);
			//preg_match('/< *img[^>]*src *= *["\']?([^"\']*)/i', $product_img_url, $matches);
			//$product_imgs = $matches;
//	        $pattern = "/(?:(?:https?|ftp|file):\/\/|www\.|ftp\.)(?:\([-A-Z0-9+&@#\/%=~_|$?!:,.]*\)|[-A-Z0-9+&@#\/%=~_|$?!:,.])*(?:\([-A-Z0-9+&@#\/%=~_|$?!:,.]*\)|[A-Z0-9+&@#\/%=~_|$])/i";
//
//	        if(preg_match_all($pattern, $product_img_url, $matches)) {
//
//	        }
