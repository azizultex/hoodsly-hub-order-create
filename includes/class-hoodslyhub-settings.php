 ...
 *
 * Further modified by wppool.dev team
 */
if ( ! class_exists( 'hoodslyhub_Settings' ) ):

	class hoodslyhub_Settings {
		...
		function callback_heading( $args ) {
			...
			$html = '<h3 class="setting_heading"><span class="setting_heading_title">' . $args['name'] . '</span><a title="' . esc_attr__( 'Click to show hide',
					'hoodslyhub' ) . '" class="setting_heading_toggle" href="#">' . esc_attr__( 'Click to show hide',
					'hoodslyhub' ) . '</a></h3>';
		...
		function callback_radio( $args ) {
			...
			foreach ( $args['options'] as $key => $label ) {
				$html .= sprintf( '<label for="wpuf-%1$s[%2$s][%3$s]">', $args['section'], $args['id'], $key );
				$html .= sprintf( '<input type="radio" class="radio %5$s" id="wpuf-%1$s[%2$s][%3$s]" name="%1$s[%2$s]" value="%3$s" %4$s />',
					$args['section'], $args['id'], $key, checked( $value, $key, false ), $args['class'] );
				...
				foreach ( $this->settings_sections as $form ) {
                    ...
                    <div id="<?php echo $form['id']; ?>" class="global_setting_group"
                        ...
                        <form method="post" action="options.php">
							<?php
							do_action( 'hoodslyhub_form_top_' . $form['id'], $form );
							settings_fields( $form['id'] );
							do_settings_sections( $form['id'] );
							do_action( 'hoodslyhub_form_bottom_' . $form['id'], $form );
							?>
                            <div style="padding-left: 10px">
								<?php submit_button( esc_html__( 'Save Settings', 'hoodslyhub' ), 'primary submit_hoodslyhub',
